...
void get_processes(Process process_stored[2]) {
    DIR *process_directory; // directory to store the information about the processes
    ...
    memset(process_stored, 0, 2 * sizeof(Process)); // Correct memory initialization
    ...
    while (1) {
        ...
        if (isdigit(*proc_entry->d_name)) { // Only process directories with numerical names (i.e., PID)
            ...
            sscanf(buffer,
                   ...
                   &current_proc.pid, current_proc.name_proc, &current_proc.utime, &current_proc.stime);
